...
class WeaponAttachment:
    ...
    @classmethod
    def from_api(cls, data: dict[str, Any], locale: str = DEFAULT_LOCALE) -> "WeaponAttachment":
...
class WeaponAttachments:
    ...
    async def get(self, name_or_slug: str) -> WeaponAttachment:
        payload = await self._client.get(f"weapon-attachments/{name_or_slug.strip()}")
        data = payload.get("data") if isinstance(payload, dict) else None
        if not data:
            raise NotFoundError(f"No weapon attachment found for {name_or_slug!r}")
        return WeaponAttachment.from_api(data, self._locale)

    async def search(self, query: str, *, limit: int = 25) -> list[WeaponAttachment]:
        query = query.strip()
        if not query:
            return []
        ...
        return [WeaponAttachment.from_api(item, self._locale) for item in items]

    async def find(self, query: str) -> Optional[WeaponAttachment]:
