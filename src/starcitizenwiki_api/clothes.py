...
class Clothes:
    ...
    async def get(self, name_or_slug: str) -> ClothingItem:
        payload = await self._client.get(f"clothes/{name_or_slug.strip()}")
        data = payload.get("data") if isinstance(payload, dict) else None
        if not data:
            raise NotFoundError(f"No clothing found for {name_or_slug!r}")
        return ClothingItem.from_api(data, self._locale)

    async def search(self, query: str, *, limit: int = 25) -> list[ClothingItem]:
        query = query.strip()
        if not query:
            return []
