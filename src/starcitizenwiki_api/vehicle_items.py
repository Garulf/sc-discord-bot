...
class VehicleItem:
    ...
    @classmethod
    def from_api(cls, data: dict[str, Any], locale: str = DEFAULT_LOCALE) -> "VehicleItem":
...
class VehicleItems:
    ...
    async def get(self, name_or_slug: str) -> VehicleItem:
        payload = await self._client.get(f"vehicle-items/{name_or_slug.strip()}")
        data = payload.get("data") if isinstance(payload, dict) else None
        if not data:
            raise NotFoundError(f"No vehicle item found for {name_or_slug!r}")
        return VehicleItem.from_api(data, self._locale)

    async def search(self, query: str, *, limit: int = 25) -> list[VehicleItem]:
        query = query.strip()
        if not query:
            return []
        ...
        return [VehicleItem.from_api(item, self._locale) for item in items]

    async def find(self, query: str) -> Optional[VehicleItem]:
