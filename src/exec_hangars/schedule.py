...
class HangarSchedule:
    ...
    @classmethod
    def from_snapshot(cls, hangar: ExecutiveHangar, observed_at: datetime | None = None) -> "HangarSchedule":
        moment = observed_at if observed_at is not None else _utcnow()
        return cls(moment - hangar.elapsed)

    @classmethod
    def from_charging(cls, lights_green: int = 0, observed_at: datetime | None = None) -> "HangarSchedule":
        return cls.from_snapshot(ExecutiveHangar.at_charging(lights_green), observed_at)

    @classmethod
    def from_active(cls, lights_expired: int = 0, observed_at: datetime | None = None) -> "HangarSchedule":
        return cls.from_snapshot(ExecutiveHangar.at_active(lights_expired), observed_at)

    @classmethod
    def from_reset(cls, elapsed_seconds: float = 0, observed_at: datetime | None = None) -> "HangarSchedule":
    ...
    def snapshot(self, now: datetime | None = None) -> ExecutiveHangar:
    ...
    def next_change(self, now: datetime | None = None) -> datetime:
        moment = self._resolve(now)
